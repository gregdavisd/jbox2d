...
//    assert(d2 != 0);
	...
	public void updateBodyContacts() {
		...
		for (int i = 0; i < m_count; i++) {
			...
			aabb.lowerBound.setMin(aabb.lowerBound, p);
			aabb.upperBound.setMax(aabb.upperBound, p);
		}
	...
	void solveRigid(final TimeStep step) {
		for (int i_group = 0; i_group < m_groupList.size(); ++i_group) {
			...
			if ((group.m_groupFlags & ParticleGroupType.b2_rigidParticleGroup) != 0) {
				...
				final Transform velocityTransform = new Transform();
	...
	void solveElastic(final TimeStep step) {
		...
		for (int k = 0; k < m_triadCount; k++) {
			...
			if ((triad.flags & ParticleType.b2_elasticParticle) != 0) {
				...
				float rs = oa.cross(pa) + ob.cross(pb) + oc.cross(pc);
				float rc = oa.dot(pa) + ob.dot(pb) + oc.dot(pc);
